...
CREATE TABLE domain_verifications (
    ...
    -- The value the owner publishes as a TXT record. Random per verification,
    -- never derived from the domain or the org id: a guessable token means
    -- anyone can predict what to publish and claim a domain they do not own.
    challenge_token     text NOT NULL,
    verified_at         timestamptz,

    -- Re-checked periodically. A domain can be transferred, expire, or be
    -- sold, and a verification that is true forever eventually certifies
    -- someone else's domain as ours.
...
-- Uploaded files.
--
-- Rows exist BEFORE the bytes do. The client asks for an upload URL, we record
-- the intent, and the client uploads directly to object storage - the API never
-- proxies image bytes, because doing so puts every upload through our memory
-- and bandwidth for no benefit.
--
-- That gap is why `confirmed_at` exists: a row without it is an upload that was
-- requested and possibly abandoned, and a sweep can delete both the row and any
-- orphaned object.
--
-- The STORAGE KEY IS GENERATED BY US, never supplied by the client. A
-- client-chosen key is a path traversal and an overwrite of someone else's
-- avatar waiting to happen.
CREATE TABLE uploads (
    id                  uuid PRIMARY KEY,
    user_id             uuid NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    -- What the file is for, so a sweep knows which bucket and which size cap
    -- applied, and so quota can be counted per purpose.
    ...
    created_at          timestamptz NOT NULL DEFAULT now(),
    CONSTRAINT uploads_purpose_check_values CHECK (
        purpose IN ('card_photo', 'org_logo', 'event_banner', 'link_image')
    )
);
CREATE UNIQUE INDEX uploads_storage_key_idx ON uploads (storage_key);
-- The sweep's queue: requested, never confirmed, old enough to be abandoned.
CREATE INDEX uploads_unconfirmed_idx ON uploads (created_at)
...
-- REVIEW NOTES
-- =============================================================================
-- Findings from the pre-migration review, executed against PostgreSQL 16.
-- Each was reproduced with a failing test before the fix.
--
-- 1. CRITICAL. users -> connections was ON DELETE CASCADE, so erasing account A
--    destroyed the connection row AND user B's private notes about the meeting.
--    That directly contradicted the retention position in ADR-0020.
--
--    Fixed by splitting `users` into an identity anchor holding no personal
--    data and `user_profiles` holding all of it. Erasure is a single DELETE of
--    the profile row. The FKs from connections are RESTRICT so the anchor
--    cannot be removed while a connection references it.
--
--    The earlier fix (anonymise the users row column by column) also worked,
--    but depended on a purge job that nulls EVERY personal column. Adding a
--    column later and forgetting the purge list would have been a silent
--    compliance defect. The split makes new personal columns covered by
--    construction.
--
-- 2. event_attendees, organization_members and event_staff used natural primary
--    keys alongside deleted_at, so anyone who left and rejoined collided with
--    their own soft-deleted row. Surrogate PK plus a partial unique index.
--
...
--    split: erasure deletes the profile row, which frees the email outright.
--
-- 5. reports cascaded from cards/users/events, so purging a suspended card
--    erased the record of why it was suspended. Now SET NULL with a
--    denormalised subject_label.
--
-- 6. entitlements accepted rows with neither value_int nor value_bool set.
--    Now a CHECK requiring exactly one.
--
-- 7. updated_at was set at INSERT and never moved. Now trigger-maintained,
--    using clock_timestamp() rather than now(): now() is the transaction start
--    time, so an insert-then-update in one transaction produced updated_at
--    equal to created_at. Found by writing the test, not by reading the code.
--
-- 8. event_roster_entries had no uniqueness, so a re-uploaded registration list
--    silently doubled the expected-attendance denominator.
--
-- Verified working and NOT changed:
--   - the connections_ordered CHECK rejects a reversed pair
--   - the two partial unique indexes correctly catch duplicate non-event
--     connections, which a single constraint over a nullable event_id would not
--
-- Accepted limitation:
...
--
-- F. anonymous_scans.saved_vcard / added_wallet replaced by scan_interactions.
--    Two booleans counted only the strongest signal and scored a scanner who
--    tapped the phone number or opened a link as a non-conversion. Screenshots
--    remain undetectable, so every figure derived from this table is a lower
